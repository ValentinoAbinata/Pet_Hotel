...
if ($action === 'list'):
    ...
    <table class="table table-striped">
      <thead><tr><th>ID</th><th>Nama</th><th>Jenis</th><th>Ras</th><th>Tgl Lahir</th><th>Pemilik</th><th>Aksi</th></tr></thead>
      <tbody>
      <?php while ($r = $q->fetch_assoc()): ?>
        <tr>
          ...
          <td>
            ...
            <a class="btn btn-sm btn-danger" href="hewan.php?action=delete&id=<?= $r['hewan_id'] ?>" onclick="return confirm('Hapus hewan ini?')">Hapus</a>
          </td>
        </tr>
      <?php endwhile; ?>
      </tbody>
  ...
  <form method="post">
    ...
    <div class="mb-3">
      ...
      <select name="customer_id" class="form-select" required>
        <?php $pelangganRes->data_seek(0); while ($p = $pelangganRes->fetch_assoc()): ?>
          <option value="<?= $p['user_id'] ?>"><?= htmlspecialchars($p['nama_lengkap']) ?></option>
        <?php endwhile; ?>
      </select>
    </div>
    <div class="mb-3"><label class="form-label">Nama hewan</label><input name="nama_hewan" class="form-control" required></div>
    ...
    <div class="mb-3"><label class="form-label">Tanggal lahir</label><input name="tanggal_lahir" type="date" class="form-control"></div>
    <div class="mb-3"><label class="form-label">Catatan pemilik</label><textarea name="catatan_pemilik" class="form-control"></textarea></div>
...
elseif ($action === 'edit' && $id):
    ...
    if (!$res || $res->num_rows==0) { flash('Hewan tidak ditemukan.'); header('Location: hewan.php?action=list'); exit; }
...
?>
  ...
  <form method="post">
    ...
    <div class="mb-3">
      ...
      <select name="customer_id" class="form-select" required>
        <?php $pelangganRes->data_seek(0); while ($p = $pelangganRes->fetch_assoc()): ?>
          <option value="<?= $p['user_id'] ?>" <?= $p['user_id']==$data['customer_id']?'selected':'' ?>><?= htmlspecialchars($p['nama_lengkap']) ?></option>
        <?php endwhile; ?>
      </select>
    </div>
    <div class="mb-3"><label class="form-label">Nama hewan</label><input name="nama_hewan" class="form-control" required value="<?= htmlspecialchars($data['nama_hewan']) ?>"></div>
    <div class="mb-3"><label class="form-label">Jenis hewan</label><input name="jenis_hewan" class="form-control" value="<?= htmlspecialchars($data['jenis_hewan']) ?>"></div>
    <div class="mb-3"><label class="form-label">Ras</label><input name="ras" class="form-control" value="<?= htmlspecialchars($data['ras']) ?>"></div>
    <div class="mb-3"><label class="form-label">Tanggal lahir</label><input name="tanggal_lahir" type="date" class="form-control" value="<?= $data['tanggal_lahir'] ?>"></div>
    <div class="mb-3"><label class="form-label">Catatan pemilik</label><textarea name="catatan_pemilik" class="form-control"><?= htmlspecialchars($data['catatan_pemilik']) ?></textarea></div>
    <button class="btn btn-primary">Simpan</button>
    <a class="btn btn-link" href="hewan.php?action=list">Batal</a>
  </form>
<?php
