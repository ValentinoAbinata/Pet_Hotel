    ...
    if ($sub === 'create') {
        ...
        if (!empty($_FILES['media']['name'])) {
            ...
            if ($f['error']===0 && in_array($f['type'],$allowed)) {
                ...
                } else $file_path = $mysqli->real_escape_string($fn);
            } else $errors[] = 'Tipe file tidak diperbolehkan atau error upload.';
        }
        ...
        if (empty($errors)) {
            ...
            if ($mysqli->query($sql)) {
                ...
                exit;
            } else $errors[] = 'Gagal menyimpan: ' . $mysqli->error;
        }
    } elseif ($sub === 'update') {
        ...
        if (!empty($_FILES['media']['name'])) {
            ...
            if ($f['error']===0 && in_array($f['type'],$allowed)) {
                ...
                $fn = 'uploads/monitoring_'.time().'_'.rand(1000,9999).'.'.$ext;
                if (move_uploaded_file($f['tmp_name'], __DIR__ . '/../'.$fn)) {
                    if ($curf && file_exists(__DIR__ . '/../'.$curf)) @unlink(__DIR__ . '/../'.$curf);
...
if ($action === 'delete' && $id) {
    ...
    if ($r && $r->num_rows) {
        $d = $r->fetch_assoc();
        if ($d['foto_video_url'] && file_exists(__DIR__ . '/../'.$d['foto_video_url'])) @unlink(__DIR__ . '/../'.$d['foto_video_url']);
    }
...
if ($action === 'list'):
    ...
    <table class="table table-striped">
      <thead><tr><th>ID</th><th>Reservasi</th><th>Tanggal</th><th>Media</th><th>Makanan</th><th>Mandi</th><th>Aksi</th></tr></thead>
      <tbody>
      <?php while ($r = $q->fetch_assoc()): ?>
        <tr>
          ...
          <td><?= $r['foto_video_url'] ? '<a href="/pet-hotel-admin/'.htmlspecialchars($r['foto_video_url']).'" target="_blank">Lihat</a>':'' ?></td>
          ...
          <td>
            ...
            <a class="btn btn-sm btn-danger" href="monitoring.php?action=delete&id=<?= $r['monitoring_id'] ?>" onclick="return confirm('Hapus record?')">Hapus</a>
          </td>
        </tr>
      <?php endwhile; ?>
      </tbody>
  ...
  <form method="post" enctype="multipart/form-data">
    ...
    <div class="mb-3">
      ...
      <select name="reservasi_id" class="form-select" required>
        <?php $resvRes->data_seek(0); while ($r = $resvRes->fetch_assoc()): ?>
          <option value="<?= $r['reservasi_id'] ?>"><?= htmlspecialchars($r['reservasi_id'].' — '.$r['nama_hewan'].' / '.$r['customer_name']) ?></option>
        <?php endwhile; ?>
      </select>
    </div>
    <div class="mb-3"><label class="form-label">Tanggal Monitoring</label><input type="date" name="tanggal_monitoring" class="form-control" value="<?= date('Y-m-d') ?>" required></div>
    <div class="mb-3"><label class="form-label">Foto/Video (jpg/png/mp4)</label><input type="file" name="media" class="form-control"></div>
    <div class="mb-3 row">
      <div class="col"><label class="form-label">Target makanan (0..3)</label><input type="number" name="target_makanan" min="0" max="3" class="form-control" value="3" required></div>
      <div class="col"><label class="form-label">Aktual makanan (0..3)</label><input type="number" name="aktual_makanan" min="0" max="3" class="form-control" value="3" required></div>
      <div class="col"><label class="form-label">Mandi?</label><select name="aktual_mandi" class="form-select"><option value="1">1 (Ya)</option><option value="0" selected>0 (Tidak)</option></select></div>
    </div>
    <div class="mb-3"><label class="form-label">Catatan Kesehatan</label><textarea name="catatan_kesehatan" class="form-control"></textarea></div>
    <div class="mb-3"><label class="form-label">Catatan Aktivitas</label><textarea name="catatan_aktivitas" class="form-control"></textarea></div>
...
elseif ($action === 'edit' && $id):
    ...
    if (!$res || $res->num_rows==0) { flash('Record tidak ditemukan.'); header('Location: monitoring.php?action=list'); exit; }
...
?>
  ...
  <form method="post" enctype="multipart/form-data">
    ...
    <div class="mb-3">
      ...
      <select name="reservasi_id" class="form-select" required>
        <?php $resvRes->data_seek(0); while ($r = $resvRes->fetch_assoc()): ?>
          <option value="<?= $r['reservasi_id'] ?>" <?= $r['reservasi_id']==$data['reservasi_id']?'selected':'' ?>><?= htmlspecialchars($r['reservasi_id'].' — '.$r['nama_hewan'].' / '.$r['customer_name']) ?></option>
        <?php endwhile; ?>
      </select>
    </div>
    <div class="mb-3"><label class="form-label">Tanggal Monitoring</label><input type="date" name="tanggal_monitoring" class="form-control" value="<?= date('Y-m-d', strtotime($data['tanggal_monitoring'])) ?>" required></div>
    <div class="mb-3">
      <label class="form-label">Foto/Video (jpg/png/mp4)</label>
      <?php if ($data['foto_video_url']): ?><p>File saat ini: <a href="/pet-hotel-admin/<?= htmlspecialchars($data['foto_video_url']) ?>" target="_blank">Lihat</a></p><?php endif; ?>
      <input type="file" name="media" class="form-control">
    </div>
    <div class="mb-3 row">
      <div class="col"><label class="form-label">Target makanan (0..3)</label><input type="number" name="target_makanan" min="0" max="3" class="form-control" value="<?= (int)$data['target_makanan'] ?>" required></div>
      <div class="col"><label class="form-label">Aktual makanan (0..3)</label><input type="number" name="aktual_makanan" min="0" max="3" class="form-control" value="<?= (int)$data['aktual_makanan'] ?>" required></div>
      <div class="col"><label class="form-label">Mandi?</label><select name="aktual_mandi" class="form-select"><option value="1" <?= $data['aktual_mandi'] ? 'selected':'' ?>>1 (Ya)</option><option value="0" <?= !$data['aktual_mandi'] ? 'selected':'' ?>>0 (Tidak)</option></select></div>
    </div>
    <div class="mb-3"><label class="form-label">Catatan Kesehatan</label><textarea name="catatan_kesehatan" class="form-control"><?= htmlspecialchars($data['catatan_kesehatan']) ?></textarea></div>
    <div class="mb-3"><label class="form-label">Catatan Aktivitas</label><textarea name="catatan_aktivitas" class="form-control"><?= htmlspecialchars($data['catatan_aktivitas']) ?></textarea></div>
    <button class="btn btn-primary">Simpan</button>
    <a class="btn btn-link" href="monitoring.php?action=list">Batal</a>
  </form>
<?php
