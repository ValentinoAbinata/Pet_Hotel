...
if ($action === 'list'):
    ...
    <table class="table">
      <thead><tr><th>ID</th><th>Nama</th><th>Email</th><th>Telp</th><th>Alamat</th><th>Aksi</th></tr></thead>
      <tbody>
      <?php while ($r = $q->fetch_assoc()): ?>
        <tr>
          ...
          <td>
            ...
            <a class="btn btn-sm btn-danger" href="pelanggan.php?action=delete&id=<?= $r['user_id'] ?>" onclick="return confirm('Hapus pelanggan?')">Hapus</a>
          </td>
        </tr>
      <?php endwhile; ?>
      </tbody>
...
elseif ($action === 'edit' && $id):
    ...
    if (!$res || $res->num_rows==0) { flash('Pelanggan tidak ditemukan.'); header('Location: pelanggan.php?action=list'); exit; }
...
?>
  ...
  <form method="post">
    ...
    <div class="mb-3"><label class="form-label">Nama lengkap</label><input name="nama_lengkap" class="form-control" value="<?= htmlspecialchars($data['nama_lengkap']) ?>" required></div>
    <div class="mb-3"><label class="form-label">Email</label><input name="email" type="email" class="form-control" value="<?= htmlspecialchars($data['email']) ?>" required></div>
    <div class="mb-3"><label class="form-label">No telepon</label><input name="no_telepon" class="form-control" value="<?= htmlspecialchars($data['no_telepon']) ?>"></div>
    <div class="mb-3"><label class="form-label">Alamat</label><textarea name="alamat" class="form-control"><?= htmlspecialchars($data['alamat']) ?></textarea></div>
