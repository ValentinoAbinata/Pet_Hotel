      ...
      <?php while ($r = $q->fetch_assoc()): ?>
        <tr>
          ...
          <td>
            ...
            <a class="btn btn-sm btn-danger" href="rekam_medis.php?action=delete&id=<?= $r['rekam_medis_id'] ?>"
              onclick="return confirm('Hapus rekam medis?')">Hapus</a>
          </td>
        </tr>
      <?php endwhile; ?>
    </tbody>
  ...
  <form method="post">
    ...
    <div class="mb-3"><label class="form-label">Hewan</label><select name="hewan_id" class="form-select"
        ...
        while ($h = $hewanRes->fetch_assoc()): ?>
          <option value="<?= $h['hewan_id'] ?>"><?= htmlspecialchars($h['nama_hewan']) ?></option><?php endwhile; ?>
      </select></div>
    <div class="mb-3"><label class="form-label">Tanggal pemeriksaan</label><input type="datetime-local"
        name="tanggal_pemeriksaan" class="form-control" value="<?= date('Y-m-d\TH:i') ?>" required></div>
    <div class="mb-3"><label class="form-label">Diagnosa</label><textarea name="diagnosa" class="form-control"></textarea>
    </div>
    <div class="mb-3"><label class="form-label">Tindakan medis</label><textarea name="tindakan_medis"
        class="form-control"></textarea></div>
    <div class="mb-3"><label class="form-label">Catatan dokter</label><textarea name="catatan_dokter"
        class="form-control"></textarea></div>
...
elseif ($action === 'edit' && $id):
  ...
  if (!$res || $res->num_rows == 0) {
    flash('Rekam medis tidak ditemukan.');
    header('Location: rekam_medis.php?action=list');
    exit;
  }
    ...
    <div class="mb-3"><label class="form-label">Hewan</label><select name="hewan_id" class="form-select"
        ...
        while ($h = $hewanRes->fetch_assoc()): ?>
          <option value="<?= $h['hewan_id'] ?>" <?= $h['hewan_id'] == $data['hewan_id'] ? 'selected' : '' ?>>
            <?= htmlspecialchars($h['nama_hewan']) ?></option><?php endwhile; ?>
      </select></div>
    <div class="mb-3"><label class="form-label">Tanggal pemeriksaan</label><input type="datetime-local"
        name="tanggal_pemeriksaan" class="form-control"
        value="<?= date('Y-m-d\TH:i', strtotime($data['tanggal_pemeriksaan'])) ?>" required></div>
    <div class="mb-3"><label class="form-label">Diagnosa</label><textarea name="diagnosa"
        class="form-control"><?= htmlspecialchars($data['diagnosa']) ?></textarea></div>
    <div class="mb-3"><label class="form-label">Tindakan medis</label><textarea name="tindakan_medis"
        class="form-control"><?= htmlspecialchars($data['tindakan_medis']) ?></textarea></div>
    <div class="mb-3"><label class="form-label">Catatan dokter</label><textarea name="catatan_dokter"
        class="form-control"><?= htmlspecialchars($data['catatan_dokter']) ?></textarea></div>
    <button class="btn btn-primary">Simpan</button>
    <a class="btn btn-link" href="rekam_medis.php?action=list">Batal</a>
  </form>
  <?php
